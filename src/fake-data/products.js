

const cf_01_image = require('../assets/img/cf-01.png')
const cf_02_image = require('../assets/img/cf-02.png')
const cf_03_image = require('../assets/img/cf-03.png')
const cf_04_image = require('../assets/img/cf-04.png')

const t_01_image = require('../assets/img/t-01.png')
const t_02_image = require('../assets/img/t-02.png')
const t_03_image = require('../assets/img/t-03.png')
const t_04_image = require('../assets/img/t-04.png')
const t_05_image = require('../assets/img/t-05.png')
const t_06_image = require('../assets/img/t-06.png')
const t_07_image = require('../assets/img/t-07.png')
const t_08_image = require('../assets/img/t-08.png')

const mt_01_image = require('../assets/img/mt-01.png')
const mt_02_image = require('../assets/img/mt-02.png')
const mt_03_image = require('../assets/img/mt-03.png')
const mt_04_image = require('../assets/img/mt-04.png')
const mt_05_image = require('../assets/img/mt-05.png')

const ds_01_image = require('../assets/img/ds-01.png')
const ds_02_image = require('../assets/img/ds-02.png')
const ds_03_image = require('../assets/img/ds-03.png')
const ds_04_image = require('../assets/img/ds-04.png')

const tp_01_image = require('../assets/img/tp-01.png')
const tp_02_image = require('../assets/img/tp-02.png')
const tp_03_image = require('../assets/img/tp-03.png')
const tp_04_image = require('../assets/img/tp-04.png')
const tp_05_image = require('../assets/img/tp-05.png')
const tp_06_image = require('../assets/img/tp-06.png')
const tp_07_image = require('../assets/img/tp-07.png')
const tp_08_image = require('../assets/img/tp-08.png')
const products =[
    {
        title:"Cà Phê Đen",
        priceOnSize:[{M:15000}],
        image:cf_01_image,
        categorySlug:"coffee",
        color:"coffee",
        description:"Cà phê nguyên chất được pha từ hạt cà phê đã được rang, xay theo tiêu chuẩn.",
        toppings:[
            // {
            //     name:"Trân Châu Đen",
            //     id:"tp-01",
            //     price:5000,
            //     image: tp_01_image,
            // },
            // {
            //     name:"Trân Hoàng Kim",
            //     id:"tp-02",
            //     price:5000,
            //     image: tp_02_image
            // },
            // {
            //     name:"Khoai Dẻo",
            //     id:"tp-03",
            //     price:7000,
            //     image: tp_03_image
            // },
            // {
            //     name:"Mứt Ôỉ",
            //     id:"tp-04",
            //     price:7000,
            //     image: tp_04_image
            // },
            
            
            // {
            //     name:"Macchiato",
            //     id:"tp-05",
            //     price:10000,
            //     image: tp_05_image
            // },
            // {
            //     name:"Cao Quy Linh",
            //     id:"tp-06",
            //     price:10000,
            //     image: tp_06_image
            // },
            // {
            //     name:"Hạt Sen",
            //     id:"tp-07",
            //     price:10000,
            //     image: tp_07_image
            // },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"cf-01",
    },
    {
        title:"Bạc Xỉu",
        priceOnSize:[{M:20000}],
        image:cf_02_image,
        categorySlug:"coffee",
        color:"coffee",
        description:"Công thức cà phê sữa, nhưng thành phần sữa sẽ nhiều hơn so với lượng cà phê.",
        toppings:[
            // {
            //     name:"Trân Châu Đen",
            //     id:"tp-01",
            //     price:5000,
            //     image: tp_01_image,
            // },
            // {
            //     name:"Trân Hoàng Kim",
            //     id:"tp-02",
            //     price:5000,
            //     image: tp_02_image
            // },
            // {
            //     name:"Khoai Dẻo",
            //     id:"tp-03",
            //     price:7000,
            //     image: tp_03_image
            // },
            // {
            //     name:"Mứt Ôỉ",
            //     id:"tp-04",
            //     price:7000,
            //     image: tp_04_image
            // },
            
            // {
            //     name:"Macchiato",
            //     id:"tp-05",
            //     price:10000,
            //     image: tp_05_image
            // },
            // {
            //     name:"Cao Quy Linh",
            //     id:"tp-06",
            //     price:10000,
            //     image: tp_06_image
            // },
            // {
            //     name:"Hạt Sen",
            //     id:"tp-07",
            //     price:10000,
            //     image: tp_07_image
            // },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"cf-02",
    },
    {
        title:"Cà Phê Sữa",
        priceOnSize:[{M:18000}],
        image:cf_03_image,
        categorySlug:"coffee",
        color:"coffee",
        description:"Một ly cà phê sữa ngon chính là hài hòa được cả vị đắng đặc trưng và ngọt từ sữa.",
        toppings:[
            // {
            //     name:"Trân Châu Đen",
            //     id:"tp-01",
            //     price:5000,
            //     image: tp_01_image,
            // },
            // {
            //     name:"Trân Hoàng Kim",
            //     id:"tp-02",
            //     price:5000,
            //     image: tp_02_image
            // },
            // {
            //     name:"Khoai Dẻo",
            //     id:"tp-03",
            //     price:7000,
            //     image: tp_03_image
            // },
            // {
            //     name:"Mứt Ôỉ",
            //     id:"tp-04",
            //     price:7000,
            //     image: tp_04_image
            // },
            
            // {
            //     name:"Macchiato",
            //     id:"tp-05",
            //     price:10000,
            //     image: tp_05_image
            // },
            // {
            //     name:"Cao Quy Linh",
            //     id:"tp-06",
            //     price:10000,
            //     image: tp_06_image
            // },
            // {
            //     name:"Hạt Sen",
            //     id:"tp-07",
            //     price:10000,
            //     image: tp_07_image
            // },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"cf-03",
    },
    {
        title:"Cà Phê Sữa Tươi",
        priceOnSize:[{M:20000}],
        image:cf_04_image,
        categorySlug:"coffee",
        color:"coffee",
        description:"Sữa tươi và cà phê nguyên chất mang hương thơm vị béo của sữa và một chút đắng nhẹ của cafe.",
        toppings:[
            // {
            //     name:"Trân Châu Đen",
            //     id:"tp-01",
            //     price:5000,
            //     image: tp_01_image,
            // },
            // {
            //     name:"Trân Hoàng Kim",
            //     id:"tp-02",
            //     price:5000,
            //     image: tp_02_image
            // },
            // {
            //     name:"Khoai Dẻo",
            //     id:"tp-03",
            //     price:7000,
            //     image: tp_03_image
            // },
            // {
            //     name:"Mứt Ôỉ",
            //     id:"tp-04",
            //     price:7000,
            //     image: tp_04_image
            // },
            
            // {
            //     name:"Macchiato",
            //     id:"tp-05",
            //     price:10000,
            //     image: tp_05_image
            // },
            // {
            //     name:"Cao Quy Linh",
            //     id:"tp-06",
            //     price:10000,
            //     image: tp_06_image
            // },
            // {
            //     name:"Hạt Sen",
            //     id:"tp-07",
            //     price:10000,
            //     image: tp_07_image
            // },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"cf-04",
    },
    {
        title:"Trà Sữa Đài Loan",
        priceOnSize:[{M:25000},{L:32000}],
        image:mt_01_image,
        categorySlug:"milktea",
        color:"milktea",
        description:"Vị trà đậm thơm nồng làm điểm nhấn nổi bật, không bị sữa lấn át mất chát trà.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            {
                name:"Phô Mai Tươi",
                id:"tp-08",
                price:10000,
                image:tp_08_image
            },
        ],
        slug:"mt-01",
    },
    {
        title:"Trà Sữa Ôlong Lài",
        priceOnSize:[{M:25000},{L:32000}],
        image:mt_02_image,
        categorySlug:"milktea",
        color:"milktea",
        description:"Trà Ô long thơm, màu sắc nhẹ, độ chát và ngọt hậu dễ gây nghiện.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            {
                name:"Phô Mai Tươi",
                id:"tp-08",
                price:10000,
                image:tp_08_image
            },
        ],
        slug:"mt-02",
    },
    {
        title:"Trà Sữa Gạo Rang",
        priceOnSize:[{M:25000},{L:32000}],
        image:mt_03_image,
        categorySlug:"milktea",
        color:"milktea",
        description:"Hương gạo thơm nhẹ, vị nguyên bản kết hợp cùng một số nguyên liệu đặc trưng.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            {
                name:"Phô Mai Tươi",
                id:"tp-08",
                price:10000,
                image:tp_08_image
            },
        ],
        slug:"mt-03",
    },
    {
        title:"Trà Sữa Thảo Mộc",
        priceOnSize:[{M:30000},{L:37000}],
        image:mt_04_image,
        categorySlug:"milktea",
        color:"milktea",
        description:"Cao quy linh hay còn gọi là quy phục linh, 1 món tráng miệng của người Hoa, màu đen, có vị đắng nhẹ.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            {
                name:"Phô Mai Tươi",
                id:"tp-08",
                price:10000,
                image:tp_08_image
            },
        ],
        slug:"mt-04",
    },
    {
        title:"Trà Sữa Uyên Ương",
        priceOnSize:[{M:25000},{L:32000}],
        image:mt_05_image,
        categorySlug:"milktea",
        color:"milktea",
        description:"Sự kết hợp tinh tế giữa trà sữa và cà phê mang đến lôi cuốn và hấp dẫn, quấn quýt không muốn rời",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            {
                name:"Phô Mai Tươi",
                id:"tp-08",
                price:10000,
                image:tp_08_image
            },
        ],
        slug:"mt-05",
    },
    {
        title:"Trà Đen Macchiato",
        priceOnSize:[{M:35000}],
        image:t_01_image,
        categorySlug:"tea",
        color:"tea",
        description:"Cũng như tình yêu, vị đắng chát của trà đen và vị ngọt béo của Macchiato tưởng không hợp mà lại hợp không tưởng.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"t-01",
    },
    {
        title:"Trà Ôlong Macchiato",
        priceOnSize:[{M:35000}],
        image:t_02_image,
        categorySlug:"tea",
        color:"tea",
        description:"Là sự kết hợp đặc biệt ở hương trà than và lớp Macchiato beo béo bồng bềnh.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"t-02",
    },
    {
        title:"Trà Hạt Sen Macchiato",
        priceOnSize:[{M:40000}],
        image:t_03_image,
        categorySlug:"tea",
        color:"tea",
        description:"Trà có vị ngọt dịu nhẹ của hạt sen pha lẫn chút béo mặn của lớp Macchiato.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"t-03",
    },
    {
        title:"Trà Trái Cây",
        priceOnSize:[{M:40000}],
        image:t_04_image,
        categorySlug:"tea",
        color:"tea",
        description:"Với cái nắng oi bức của mặt trời thì trà trái cây được xem như là một cách để giải nhiệt.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            {
                name:"Phô Mai Tươi",
                id:"tp-08",
                price:10000,
                image:tp_08_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image:`/assets/img/tp-07.png`
            },
            // {
            //     name:"Mứt Ổi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"t-04",
    },
    {
        title:"Trà Ổi Hồng",
        priceOnSize:[{M:35000}],
        image:t_05_image,
        categorySlug:"tea",
        color:"tea",
        description:"Với cái nắng oi bức của mặt trời thì trà trái cây được xem như là một cách để giải nhiệt.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"t-05",
    },
    {
        title:"Trà Đen Vải Thiều",
        priceOnSize:[{M:30000}],
        image:t_06_image,
        categorySlug:"tea",
        color:"tea",
        description:"Trà đen vải thiều mang vị ngọt thanh của vải kết hợp với vị chua của chanh và hương trà đen thơm nhẹ.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"t-06",
    },
    {
        title:"Trà Nhãn",
        priceOnSize:[{M:35000}],
        image:t_07_image,
        categorySlug:"tea",
        color:"tea",
        description:"Những trái nhãn tươi có vị ngọt, mùi thơm đặc trưng thanh mát, giải nhiệt.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"t-07",
    },{
        title:"Trà Đen Thảo Mộc",
        priceOnSize:[{M:35000}],
        image:t_08_image,
        categorySlug:"tea",
        color:"tea",
        description:"Trà có vị đắng nhẹ của trà đen và cao quy linh. Dành cho những ai ghét màu hồng, yêu bóng tối.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"t-08",
    },
    {
        title:"Chè Thảo Mộc",
        priceOnSize:[{M:40000},{L:50000}],
        image:ds_01_image,
        categorySlug:"desert",
        color:"desert",
        description:"Cùng với khoai dẻo, trân châu, vị ngọt của kem béo và ngọt thanh của đá thảo mộc sẽ giảm bớt vị đắng của cao quy linh.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"ds-01",
    },
    {
        title:"Chè Đậu Đỏ",
        priceOnSize:[{M:35000},{L:45000}],
        image:ds_02_image,
        categorySlug:"desert",
        color:"desert",
        description:"Sự kết hợp giữa đậu đỏ và sương sáo, thêm khoai dẻo mang đến sự bùng nổ vị giác.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"ds-02",
    },
    {
        title:"Trứng Nấu Trà Đen",
        priceOnSize:[{M:20000},{L:25000}],
        image:ds_03_image,
        categorySlug:"desert",
        color:"desert",
        description:"Trứng nấu trà đen mang hương thơm đặc trưng từ các loại thảo mộc, là một món ăn bổ dưỡng của người Đài Loan.",
        toppings:[
            // {
            //     name:"Trân Châu Đen",
            //     id:"tp-01",
            //     price:5000,
            //     image: tp_01_image,
            // },
            // {
            //     name:"Trân Hoàng Kim",
            //     id:"tp-02",
            //     price:5000,
            //     image: tp_02_image
            // },
            // {
            //     name:"Khoai Dẻo",
            //     id:"tp-03",
            //     price:7000,
            //     image: tp_03_image
            // },
            // {
            //     name:"Mứt Ôỉ",
            //     id:"tp-04",
            //     price:7000,
            //     image: tp_04_image
            // },
            
            // {
            //     name:"Macchiato",
            //     id:"tp-05",
            //     price:10000,
            //     image: tp_05_image
            // },
            // {
            //     name:"Cao Quy Linh",
            //     id:"tp-06",
            //     price:10000,
            //     image: tp_06_image
            // },
            // {
            //     name:"Hạt Sen",
            //     id:"tp-07",
            //     price:10000,
            //     image: tp_07_image
            // },
            // {
            //     name:"Phô Mai Tươi",
            //     id:"tp-08",
            //     price:10000,
            //     image:tp_08_image
            // },
        ],
        slug:"ds-03",
    },
    {
        title:"Tàu Hủ Singapore",
        priceOnSize:[{M:18000}],
        image:ds_04_image,
        categorySlug:"desert",
        color:"desert",
        description:"Tàu hủ Singapore mang vị đặc trưng của đậu nành ngọt nhẹ ăn kèm với topping tùy sở thích.",
        toppings:[
            {
                name:"Trân Châu Đen",
                id:"tp-01",
                price:5000,
                image: tp_01_image,
            },
            {
                name:"Trân Hoàng Kim",
                id:"tp-02",
                price:5000,
                image: tp_02_image
            },
            {
                name:"Khoai Dẻo",
                id:"tp-03",
                price:7000,
                image: tp_03_image
            },
            {
                name:"Mứt Ôỉ",
                id:"tp-04",
                price:7000,
                image: tp_04_image
            },
            
            {
                name:"Macchiato",
                id:"tp-05",
                price:10000,
                image: tp_05_image
            },
            {
                name:"Cao Quy Linh",
                id:"tp-06",
                price:10000,
                image: tp_06_image
            },
            {
                name:"Hạt Sen",
                id:"tp-07",
                price:10000,
                image: tp_07_image
            },
            {
                name:"Phô Mai Tươi",
                id:"tp-08",
                price:10000,
                image:tp_08_image
            },
        ],
        slug:"ds-04",
    },
]

const getAllProducts = () => products

const getRandomProducts = (count) => {
    const max = products.length - count
    const min = 0
    const start = Math.floor(Math.random() * (max - min) + min)
    return products.slice(start, start + count)
}

const getProductBySlug = (slug) => products.find(e => e.slug === slug)

const getCartItemsInfo = (cartItems) => {
    let res = []
    if (cartItems.length > 0) {
        cartItems.forEach(e => {
            let product = getProductBySlug(e.slug)
            res.push({
                ...e,
                product: product
            })
        })
    }
    // console.log(res)
    // console.log('sorted')
    // console.log(res.sort((a, b) => a.slug > b.slug ? 1 : (a.slug < b.slug ? -1 : 0)))
    return res.sort((a, b) => a.id > b.id ? 1 : (a.id < b.id ? -1 : 0))
}

const productData = {
    getAllProducts,
    getRandomProducts,
    getProductBySlug,
    getCartItemsInfo
}

export default productData